...
<template>
  <div class="game-manage manage-main" :class="isSearchShow ? 'is-hidden' : ''">
    ...
    <el-table :data="tableList" class="manage-table" style="" @selection-change="selectionChange">
      ...
      <el-table-column label="游戏名" align="center" prop="name" show-overflow-tooltip />
  ...
  <c-dialog v-model="open" :title="title" width="500" :modal="true">
    <el-form :model="form" label-width="60">
      ...
      <el-form-item label="封面">
        <upload v-if="!fromNet" v-model="form.coverUrl" path="game" />
...
async function handleDel() {
  ElMessageBox.confirm('是否确认删除选中数据?', 'Warning', {
    ...
    let ids = [];
    ids = selectIds.value;
    const { code, msg, data } = (await delGame(ids)) as any;
    if (code == 200) {
      ElNotification({
        title: 'Success',
        message: '删除成功',
